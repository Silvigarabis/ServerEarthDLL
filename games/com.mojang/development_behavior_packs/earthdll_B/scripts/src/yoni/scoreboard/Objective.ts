...
class Objective {
     ...
     * @param {Entry|Minecraft.ScoreboardIdentity|Minecraft.Entity|Minecraft.Player|string|number|YoniEntity} entry - 可能为分数持有者的值。
     */
    static findCommandRequirement(one): {name?: string, type: string, entity?: EntityBase}{
        let name = null;
        let type = null;
        let entity = null;
        let scbid = (one instanceof Minecraft.ScoreboardIdentity) ? one : null;
        let entry = (one instanceof Entry) ? one : null;
        ...
        if (scbid != null || entry != null){
            ...
            return Objective.findCommandRequirement(Entry.getEntry({id: one}));
        } else {
     ...
     */
    getScore(one){
        let entry = Entry.guessEntry(one);
    ...
    async resetAllScores(){
        await Command.add(Command.PRIORITY_HIGHEST, Command.getCommandMoreStrict("scoreboard", "players", "reset", "*", this.#id));
    }
